            ...
            try:
                ...
                with sync_playwright() as playwright:
                    ...
                    with zipfile.ZipFile(archive_path) as archive:
                        ...
                        assert manifest["config"]["privacy_blur_enabled"] is False
                        assert manifest["config"]["privacy_reason"].startswith(
                    ...
                    page.get_by_role("radio", name="Monitor", exact=True).click()
                    page.get_by_role("combobox", name="Sample scene", exact=True).click()
                    page.get_by_role(
                        "option", name="Piling works · access zone", exact=True
                    ).click()
                    page.get_by_role("button", name="Run included sample", exact=True).click()
                    expect(
                    ...
                    page.get_by_role("radio", name="Monitor", exact=True).click()
                    page.get_by_role("radiogroup", name="Source type").get_by_text(
                        "Image", exact=True
                    ).click()
                    page.locator("input[type=file]").set_input_files(
                        str(root / "examples/sample_images/construction_site_ppe_01.jpg")
                    )
                    page.get_by_role("button", name="Run safety analysis", exact=True).click()
                    expect(page.get_by_text("Frames analysed", exact=True)).to_be_visible()
                    with page.expect_download() as upload_download:
                        page.get_by_role(
                            "button", name="Download complete report", exact=True
                        ).click()
                    upload_archive = Path(temporary) / "upload-report.zip"
                    upload_download.value.save_as(upload_archive)
                    with zipfile.ZipFile(upload_archive) as archive:
                        assert (
                            json.loads(archive.read("manifest.json"))["config"][
                    ...
                    page.get_by_role("radio", name="Monitor", exact=True).click()
                    page.get_by_role("radiogroup", name="Source type").get_by_text(
                        "Sample scenes", exact=True
                    ).click()
                    mobile = browser.new_page(viewport={"width": 390, "height": 844})
                    mobile.goto(f"http://127.0.0.1:{port}")
                    expect(
                        mobile.get_by_role("button", name="Run included sample", exact=True)
                    ).to_be_visible()
                    assert mobile.evaluate("document.documentElement.scrollWidth <= innerWidth + 1")
                    mobile.screenshot(path=str(args.output / "mobile-v2.2.png"), full_page=True)
                    browser.close()
                print(
                    "PASS: reviewed rear-view sample and zone event, upload privacy isolation, real PPE inference, report hashes, H.264 playback and mobile layout."
                )
            finally:
                server.terminate()
                try:
