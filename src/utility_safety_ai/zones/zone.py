...
class Zone:
    ...
    def __post_init__(self) -> None:
        ...
        if self.risk_level not in RISK_LEVELS:
            raise ValueError(f"Zone {zone_id!r} risk_level must be one of {sorted(RISK_LEVELS)}")
        if self.coordinate_space not in COORDINATE_SPACES:
            raise ValueError("Zone coordinate_space must be either 'pixels' or 'normalized'")
        ...
        for index, point in enumerate(self.polygon):
            if not isinstance(point, (list, tuple)) or len(point) != 2:
                raise ValueError(f"Zone {zone_id!r} polygon point {index} must contain x and y")
            x, y = point
            if isinstance(x, bool) or isinstance(y, bool):
                raise ValueError(f"Zone {zone_id!r} polygon point {index} must be numeric")
            try:
                normalized_point = (float(x), float(y))
            except (TypeError, ValueError) as exc:
                raise ValueError(f"Zone {zone_id!r} polygon point {index} must be numeric") from exc
            if not all(isfinite(value) for value in normalized_point):
                raise ValueError(f"Zone {zone_id!r} polygon point {index} must be finite")
            if self.coordinate_space == "normalized" and not all(
                0.0 <= value <= 1.0 for value in normalized_point
            ):
                raise ValueError(f"Zone {zone_id!r} normalized coordinates must be in [0, 1]")
        ...
        if isinstance(self.required_ppe, str) or not isinstance(self.required_ppe, (list, tuple)):
        ...
        if any(not isinstance(item, str) for item in self.required_ppe):
            raise ValueError(f"Zone {zone_id!r} required_ppe values must be strings")
        ...
        if isinstance(self.dwell_seconds, bool) or not isinstance(self.dwell_seconds, (int, float)):
        ...
        if not isfinite(dwell_seconds) or dwell_seconds < 0:
            raise ValueError(f"Zone {zone_id!r} dwell_seconds must be a finite non-negative value")
    ...
    def resolved_polygon(
        ...
        if frame_size is None:
            raise ValueError(f"Normalized zone {self.id!r} requires frame_size=(width, height)")
